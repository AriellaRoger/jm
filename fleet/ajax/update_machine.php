<?php
// File: fleet/ajax/update_machine.php
// Update existing company machine
...
// Only administrators and supervisors can update machines
...
try {
    ...
    if (empty($input['machine_id']) || empty($input['machine_name']) || empty($input['machine_type']) || empty($input['branch_id'])) {
        throw new Exception('Machine ID, machine name, machine type, and branch are required');
    }

    $machineId = intval($input['machine_id']);
    ...
    $result = $fleetController->updateMachine($machineId, $machineData, $_SESSION['user_id']);
