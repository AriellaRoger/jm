<?php
// File: inventory/ajax/update_third_party_product.php
// AJAX handler for updating third party products (Admin/Supervisor only)
...
$requiredFields = ['name', 'brand', 'unit_of_measure', 'cost_price', 'selling_price', 'status'];
...
try {
    ...
    $result = $inventoryController->updateThirdPartyProduct($productId, $data);

    if ($result) {
        echo json_encode([
            'success' => true,
            'message' => 'Third party product updated successfully'
        ]);
    } else {
        echo json_encode(['success' => false, 'message' => 'Failed to update third party product']);
    }
} catch (Exception $e) {
